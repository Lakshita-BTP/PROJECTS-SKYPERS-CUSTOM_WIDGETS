(function () {
  class MilestoneOutstanding extends HTMLElement {
    constructor() {
      ...
      this._selectedInvoiced = 0;
      this._selectedOutstanding = 0;
    ...
    getSelectedInvoiced() {
      return this._selectedInvoiced;
    }

    getSelectedOutstanding() {
      return this._selectedOutstanding;
    }

    fireRowSelect() {
      this.dispatchEvent(new Event("onRowSelect"));
    }

    fireTotalInvoicedSelect(totalInvoiced) {
      this._selectedInvoiced = totalInvoiced;
      this.dispatchEvent(new Event("onTotalInvoicedSelect"));
    }

    fireTotalOutstandingSelect(totalOutstanding) {
      this._selectedOutstanding = totalOutstanding;
      this.dispatchEvent(new Event("onTotalOutstandingSelect"));
    }

    getSelectedTotalInvoiced() {
      return this._selectedInvoiced;
    }

    getSelectedTotalOutstanding() {
      return this._selectedOutstanding;
    }
    ...
    render() {
      ...
      try {
        ...
        data.forEach((item) => {
          ...
          <div
            class="row milestone-row"
            data-milestone="${item.milestone}"
            data-invoiced="${item.invoiced}"
        ...
        content.innerHTML = `
          ...
          <div class="footer">
            ...
            <div
              class="total-box total-invoiced-box"
            >
            ...
            <div
              class="total-box total-outstanding-box"
            >
        ...
        const rowsSelected = content.querySelectorAll(".milestone-row");
        rowsSelected.forEach((row) => {
          row.style.cursor = "pointer";
          row.addEventListener("click", () => {
            this._selectedMilestone = row.dataset.milestone;
            this._selectedInvoiced = Number(row.dataset.invoiced);
            this._selectedOutstanding = Number(row.dataset.outstanding);
            this.fireRowSelect();
          });
        });
        ...
        const invoicedBox = content.querySelector(".total-invoiced-box");

        if (invoicedBox) {
          invoicedBox.style.cursor = "pointer";

          invoicedBox.addEventListener("click", () => {
            this.fireTotalInvoicedSelect(totalInvoiced);
          });
        }
        ...
        const outstandingBox = content.querySelector(".total-outstanding-box");

        if (outstandingBox) {
          outstandingBox.style.cursor = "pointer";

          outstandingBox.addEventListener("click", () => {
            this.fireTotalOutstandingSelect(totalOutstanding);
          });
        }
      } catch (e) {
        content.innerHTML = `
    ...
    async serializeCustomWidgetToImage() {
      ...
      const availableRowsHeight =
        canvasHeight - headerHeight - tableHeaderHeight - footerReservedHeight;
