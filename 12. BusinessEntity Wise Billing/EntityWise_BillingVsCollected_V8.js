...
async serializeCustomWidgetToImage() {
  ...
  const width =
    this.shadowRoot.host.clientWidth || this.clientWidth || 900;
  ...
  const barWidth = width - leftPadding - rightPadding - labelWidth - valueWidth - gap;
  ...
  rowsToDraw.forEach((item, index) => {
    ...
    const percent =
      item.billing > 0
        ? (item.collected / item.billing) * 100
        : 0;
    ...
    words.forEach((word) => {
      ...
      if (
        ctx.measureText(test).width <= labelWidth - 5
      ) {
        line = test;
      } else {
        lines.push(line);
