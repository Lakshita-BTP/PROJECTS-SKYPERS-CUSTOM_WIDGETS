(function () {
  class TuspValueWithoutTax extends HTMLElement {
    ...
    async serializeCustomWidgetToImage() {
      ...
      const width = this.shadowRoot.host.clientWidth || 800;
      const height = this.shadowRoot.host.clientHeight || 500;
      ...
      /*----------------------------------
          Sizes (Responsive)
      -----------------------------------*/
      const headerH = height * 0.095;
      const outerPad = width * 0.02;
      const bodyTop = headerH + height * 0.04;
      //const titleFont = Math.max(10, width * 0.028 );
      const titleFont = Math.max(10, width * 0.038 );
      const mainFont = Math.max(22, Math.min(width * 0.085, height * 0.1));
      const subFont = Math.max(8, width * 0.022);
      const bucketValueFont = Math.max(10, width * 0.03);
      const bucketLabelFont = Math.max(10, width * 0.028);
      const avgLabelFont = Math.max(9, width * 0.022);
      const avgValueFont = Math.max(12, width * 0.032);
      ...
      const startY = y + height * 0.06;
      const buckets = [
        [this.formatNumber(this._bucket0_30), "0-30"],
        [this.formatNumber(this._bucket31_60), "31-60"],
        [this.formatNumber(this._bucket61_90), "61-90"],
        [this.formatNumber(this._bucket90Plus), "Above 90"],
      ];
      ...
      /*----------------------------------
          Avg Box
      -----------------------------------*/
      const avgY = startY + (boxH + gap) * 2 + height * 0.05;
      ...
      ctx.fillText("Avg Sales Price", width / 2, avgY + avgH * 0.3);

      ctx.fillStyle = "#16263B";
      ctx.font = `bold ${avgValueFont}px Arial`;

      ctx.font = `bold ${avgValueFont}px Arial`;
      const value = this.formatNumber(this._avgSalesPrice);
      const valueWidth = ctx.measureText(value).width;

      ctx.fillStyle = "#16263B";
      ctx.fillText(value, width / 2 - 18, avgY + avgH * 0.68);

      ctx.font = `${avgLabelFont}px Arial`;
      ctx.fillStyle = "#7D8CA3";
      ctx.fillText(
        this._avgUnit,
        width / 2 + valueWidth / 2 + 6,
        avgY + avgH * 0.68,
      );
