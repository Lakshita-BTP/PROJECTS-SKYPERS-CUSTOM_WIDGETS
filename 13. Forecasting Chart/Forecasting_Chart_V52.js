  ...
  class PaymentForecastDashboard extends HTMLElement {
    constructor() {
      ...
      /*==================================================
          TITLE
      ==================================================*/
      this._title = "PLANNED CASHFLOW FORECAST";
    ...
    FINANCIAL YEARS
    ==================================================*/
    getNextFinancialYears(count) {
      const today = new Date();

      // Current financial year starts on 1 April
      const fyStartYear =
    ...
    render() {
      ...
      try {
        ...
        console.log("Metadata:", this._myDataBinding.metadata);
        ...
        /*------------------------------------------
              HISTORY DATA
        ------------------------------------------*/
        const Keysmeasure = this._myDataBinding.metadata.feeds.measures.values;

        // Replace "Financial Year" with your actual dimension key
        const yearKey = this._myDataBinding.metadata.feeds.dimensions.values[0];

        const nextFiveYears = this.getNextFinancialYears(4); // Current FY + next 4 = 5 years

        this._historyData = rows
          .filter((row) => nextFiveYears.includes(row[yearKey]?.label))
          .map((row) => {
            let total = 0;
            ...
            return {
              year: row[yearKey]?.label,
              value: total,
            };
          });

        console.log("Rows Length:", rows.length);
        console.log("History Data:", this._historyData);
    ...
    async serializeCustomWidgetToImage() {
      ...
      ctx.fillText("Future Billing Forecast", leftX, topY);
