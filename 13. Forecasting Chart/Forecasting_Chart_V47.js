  ...
  class PaymentForecastDashboard extends HTMLElement {
    ...
    render() {
      ...
      try {
        ...
        const historyRows = rows.slice(-5);

        // Generate exactly 5 financial years
        const financialYears = this.getNextFinancialYears(historyRows.length);

        this._historyData = historyRows.map((row, index) => {
          let total = 0;
