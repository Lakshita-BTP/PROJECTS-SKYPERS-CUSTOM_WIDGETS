(function () {
  class PaymentForecastDashboard extends HTMLElement {
    ...
    async serializeCustomWidgetToImage() {
      ...
      /* Hide last bucket (GT 365 Days) */
      const forecastItems = this._forecastData.slice(0, -1);

      const totalForecastValue = forecastItems.reduce(
        (sum, item) => sum + item.value,
        0,
      );

      /* Layout */
      const stackTop = topY + 40;
      const stackHeight = 24;
      const stackWidth = leftWidth;
      const labelGap = 42;
      ...
      =============================*/
      let currentX = leftX;
      forecastItems.forEach((bucket, index) => {
        const color = this._barColors[index] || "#2563EB";
        const segmentWidth =
          totalForecastValue === 0
            ? 0
            : (bucket.value / totalForecastValue) * stackWidth;
        ...
        if (index === 0 || index === forecastItems.length - 1) {
          ctx.roundRect(currentX, stackTop, segmentWidth, stackHeight, 10);
        } else {
          ctx.rect(currentX, stackTop, segmentWidth, stackHeight);
        }
      ...
      forecastItems.forEach((bucket, index) => {
        ...
        /* Color dot */
        ctx.beginPath();
        ctx.arc(leftX + 6, rowY - 4, 5, 0, Math.PI * 2);
        ctx.fillStyle = color;
        ctx.fill();
        ...
        ctx.fillText(bucket.label, leftX + 18, rowY);
        ...
        ctx.fillText(percent.toFixed(1) + "%", leftX + leftWidth - 95, rowY);
      ...
      /* Chart Area */
      const chartX = rightX;
      const chartY = topY + 30;
      const chartWidth = rightWidth;
      const chartHeight = Math.max(260, availableHeight - 30);
      ...
      this._historyData.forEach((item, index) => {
        ...
        const y = graphY + graphHeight - (item.value / maxValue) * graphHeight;
