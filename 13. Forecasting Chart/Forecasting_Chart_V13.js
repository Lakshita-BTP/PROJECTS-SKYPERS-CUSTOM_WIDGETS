(function () {
  class PaymentForecastDashboard extends HTMLElement {
    ...
    async serializeCustomWidgetToImage() {
      ...
      // Left (Forecast)
      const leftX = margin;
      const leftWidth = Math.max(260, width * 0.36);
      ...
      this._forecastData.forEach((bucket, index) => {
        ...
        ctx.beginPath();
        ctx.roundRect(leftX, y, leftWidth, 16, 8);
        ctx.fill();
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.roundRect(
        ...
        ctx.fillStyle = "#667085";
        ctx.font = "10px Arial";
        ctx.fillText(share.toFixed(1) + "%", leftX, y + 30);
      ...
      const chartHeight = availableHeight - 25;
