(function () {
  class SalesTableWidget extends HTMLElement {
    constructor() {
      super();
    ...
    editTitle() {
      const newTitle = prompt("Enter new title:", this._titleText);

      if (newTitle !== null && newTitle.trim() !== "") {
        this._titleText = newTitle.trim();
        this.render();
      }
    }
    ...
    /* =========================
      HIDE MEASURES
    ========================= */
    hideMeasures(measureNames) {
      this._hiddenMeasures = String(measureNames)
        .split(",")
        .map((m) => m.trim())
        .filter(Boolean);
    ...
    render() {
      ...
      try {
        ...
        /* =========================
          HIDE MEASURES
        ========================= */
        if (this._hiddenMeasures.length > 0) {
          measures = measures.filter((measure) => {
            ...
            return !(
              this._hiddenMeasures.includes(measure) ||
              this._hiddenMeasures.includes(label)
            );
          });
        }

        /* =========================
          SHOW ONLY SELECTED MEASURES
        ========================= */
  ...
  customElements.define("com-max-table-salestable", SalesTableWidget);
})();
